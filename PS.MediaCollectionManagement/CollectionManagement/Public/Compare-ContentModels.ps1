...
function Compare-ContentModels (
    ...
    if ($PSBoundParameters.ContainsKey('baselineFilePath')) {
        ...
        if (-not (([Bool]($incomingA.PSObject.Properties.Name -match "FileType")) -and 
                  ...
                  ($incomingA.FileType -eq [CollectionManagementDefaults]::DEFAULT_MODULE_FILETYPE()))) {
            Write-ErrorToConsole "Error: Baseline is not a valid" ([CollectionManagementDefaults]::DEFAULT_MODULE_FILETYPE()) "filetype."
            return
        }
    ...
    if ($PSBoundParameters.ContainsKey('comparisonFilePath')) {
        ...
        if (-not (([Bool]($incomingB.PSObject.Properties.Name -match "FileType")) -and 
                  ...
                  ($incomingB.FileType -eq [CollectionManagementDefaults]::DEFAULT_MODULE_FILETYPE()))) {
            Write-ErrorToConsole "Error: Comparison is not a valid" ([CollectionManagementDefaults]::DEFAULT_MODULE_FILETYPE()) "filetype."
            return
        }
