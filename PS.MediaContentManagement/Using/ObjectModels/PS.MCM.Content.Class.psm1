    ...
    [void] Hidden Init(
        ...
        if ($splitBaseName.Count -ne $config.FilenameFormat.Count) {
            ...
            if ($seasonEpisodeSplitIndex -ge 0) {
                if ([ElementParser]::IsValidSeasonEpisode($splitBaseName[$seasonEpisodeSplitIndex].Trim())) {
                    
                    $this.Season = [ElementParser]::GetSeason($splitBaseName[$seasonEpisodeSplitIndex].Trim())
                    $this.Episode = [ElementParser]::GetEpisode($splitBaseName[$seasonEpisodeSplitIndex].Trim())
            ...
            if ($trackSplitIndex -ge 0) {
                if ([ElementParser]::IsValidTrackNumber($splitBaseName[$trackSplitIndex].Trim())) {
            ...
            if ($yearSplitIndex -ge 0) {
                if ([ElementParser]::IsValidYear($splitBaseName[$trackSplitIndex].Trim())) {
    ...
    [Void] FillPropertiesWhereMissing([System.IO.FileInfo] $file, [Bool] $silently) {
        ...
        try {
            ...
            switch ($this.Extension) {
                {$_ -in [ModuleSettings]::DEFAULT_VIDEO_EXTENSIONS()} {
                ...
                {$_ -in [ModuleSettings]::DEFAULT_AUDIO_EXTENSIONS()} {
