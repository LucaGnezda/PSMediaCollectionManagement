...
class ContentModel {
    ...
    [Bool] AlterActor ([String] $fromName, [String] $toName, [Bool] $updateCorrespondingFilename) {
        ...
        <#
        # First check Actors is not null
        if ($null -eq $this.Actors) {
            Write-WarnToConsole "Actors are not initialised on this ContentModel, adandoning action."
            return $false
        }

        # Set starting state
        [Int]   $alterations = 0
        [Actor] $fromObject = ($this.Actors | Where-Object {$_.Name -ceq $fromName})
        [Actor] $toObject = ($this.Actors | Where-Object {$_.Name -ceq $toName})
        ...
        foreach ($item in $fromObject.PerformedIn) {

            # determine the end state base name
            $splitBaseName = ($item.BaseName -split $this.Config.FilenameSplitter).Trim()
            $actorsSplitIndex = $this.Config.FilenameFormat.IndexOf([FilenameElement]::Actors)
            $splitBaseName[$actorsSplitIndex] = $splitBaseName[$actorsSplitIndex].Replace($fromName, $toName)
        ...
        # if there is no toObject, then we can just renamne the existing object
        if ($null -eq $toObject) {
        ...
        foreach ($contentItem in $fromObject.PerformedIn) {
            ...
            if ($null -ne $toObject) {
                ...
                # and if its not the same object
                if ($fromObject -ne $toObject) {

                    # remap content references
                    for ($i = 0; $i -lt $contentItem.Actors.Count; $i++) {
                        if ($contentItem.Actors[$i].Name -eq $fromName) {
                            $contentItem.Actors[$i] = $toObject
                        }
                    }

                    # add the content to the toObject
                    $toObject.PerformedIn.Add($contentItem)
                }
...
#>
    ...
    [Bool] AlterAlbum ([String] $fromName, [String] $toName, [Bool] $updateCorrespondingFilename) {
        ...
        <#       
        # First check Albums is not null
        if ($null -eq $this.Albums) {
            Write-WarnToConsole "Series are not initialised on this ContentModel, adandoning action."
            return $false
        }

        # Set starting state
        [Int]   $alterations = 0
        [Album] $fromObject = ($this.Albums | Where-Object {$_.Name -ceq $fromName})
        [Album] $toObject = ($this.Albums | Where-Object {$_.Name -ceq $toName})
        ...
        # Lets make sure we won't cause a content colision. So ... foreach 'from Content' 
        foreach ($item in $fromObject.Tracks) {

            # determine the end state base name
            $splitBaseName = ($item.BaseName -split $this.Config.FilenameSplitter).Trim()
            $albumSplitIndex = $this.Config.FilenameFormat.IndexOf([FilenameElement]::Album)
            $splitBaseName[$albumSplitIndex] = $splitBaseName[$albumSplitIndex].Replace($fromName, $toName)
            $plannedBaseName = $splitBaseName -join $this.Config.FilenameSplitter
        ...
        # process each content item connected to the fromObject
        foreach ($contentItem in $fromObject.Tracks) {
            ...
            if ($null -ne $toObject) {
                ...
                if ($fromObject -ne $toObject) {
                    ...
                    $contentItem.OnAlbum = $toObject

                    # add the content to the toObject
                    $toObject.Tracks.Add($contentItem)
...
#>
    ...
    [Bool] AlterArtist ([String] $fromName, [String] $toName, [Bool] $updateCorrespondingFilename) {
        ...
        <#       
        # First check Artists is not null
        if ($null -eq $this.Artists) {
            Write-WarnToConsole "Actors are not initialised on this ContentModel, adandoning action."
            return $false
        }
        ...
        # Set starting state
        [Int]   $alterations = 0
        [Artist] $fromObject = ($this.Artists | Where-Object {$_.Name -ceq $fromName})
        [Artist] $toObject = ($this.Artists | Where-Object {$_.Name -ceq $toName})
        ...
        # Lets make sure we won't cause a content colision. So ... foreach 'from Content' 
        foreach ($item in $fromObject.Performed) {

            # determine the end state base name
            $splitBaseName = ($item.BaseName -split $this.Config.FilenameSplitter).Trim()
            $artistsSplitIndex = $this.Config.FilenameFormat.IndexOf([FilenameElement]::Artists)
            $splitBaseName[$artistsSplitIndex] = $splitBaseName[$artistsSplitIndex].Replace($fromName, $toName)
            $plannedBaseName = $splitBaseName -join $this.Config.FilenameSplitter
        ...
        # process each content item connected to the fromObject
        foreach ($contentItem in $fromObject.Performed) {
            ...
            if ($null -ne $toObject) {
                ...
                if ($fromObject -ne $toObject) {
                    ...
                    # remap content references
                    for ($i = 0; $i -lt $contentItem.Artists.Count; $i++) {
                        if ($contentItem.Artists[$i].Name -eq $fromName) {
                            $contentItem.Artists[$i] = $toObject
                        }
                    }

                    # add the content to the toObject
                    $toObject.Performed.Add($contentItem)
                }
        ...
        if (($null -ne $toObject) -and ($fromObject -ne $toObject)) {
            ...
            $this.Artists.Remove($fromObject)
        }
...
#>
    ...
    [Bool] AlterSeries ([String] $fromName, [String] $toName, [Bool] $updateCorrespondingFilename) {
        ...
        <#
        # First check Series is not null
        if ($null -eq $this.Series) {
            Write-WarnToConsole "Series are not initialised on this ContentModel, adandoning action."
            return $false
        }
        ...
        # Set starting state
        [Int]   $alterations = 0
        [Series] $fromObject = ($this.Series | Where-Object {$_.Name -ceq $fromName})
        [Series] $toObject = ($this.Series | Where-Object {$_.Name -ceq $toName})
        ...
        # Lets make sure we won't cause a content colision. So ... foreach 'from Content' 
        foreach ($item in $fromObject.Episodes) {

            # determine the end state base name
            $splitBaseName = ($item.BaseName -split $this.Config.FilenameSplitter).Trim()
            $seriesSplitIndex = $this.Config.FilenameFormat.IndexOf([FilenameElement]::Series)
            $splitBaseName[$seriesSplitIndex] = $splitBaseName[$seriesSplitIndex].Replace($fromName, $toName)
            $plannedBaseName = $splitBaseName -join $this.Config.FilenameSplitter
        ...
        # process each content item connected to the fromObject
        foreach ($contentItem in $fromObject.Episodes) {
            ...
            if ($null -ne $toObject) {
                ...
                if ($fromObject -ne $toObject) {
                    ...
                    # remap content reference
                    $contentItem.FromSeries = $toObject

                    # add the content to the toObject
                    $toObject.Episodes.Add($contentItem)
...
<#        
        # First check Studios is not null
        if ($null -eq $this.Studios) {
            Write-WarnToConsole "Studios are not initialised on this ContentModel, adandoning action."
            return $false
        }

        # Set starting state
        [Int]   $alterations = 0
        [Studio] $fromObject = ($this.Studios | Where-Object {$_.Name -ceq $fromName})
        [Studio] $toObject = ($this.Studios | Where-Object {$_.Name -ceq $toName})
        ...
        # Lets make sure we won't cause a content colision. So ... foreach 'from Content' 
        foreach ($item in $fromObject.Produced) {

            # determine the end state base name
            $splitBaseName = ($item.BaseName -split $this.Config.FilenameSplitter).Trim()
            $studioSplitIndex = $this.Config.FilenameFormat.IndexOf([FilenameElement]::Studio)
            $splitBaseName[$studioSplitIndex] = $splitBaseName[$studioSplitIndex].Replace($fromName, $toName)
            $plannedBaseName = $splitBaseName -join $this.Config.FilenameSplitter
        ...
        # process each content item connected to the fromObject
        foreach ($contentItem in $fromObject.Produced) {
            ...
            if ($null -ne $toObject) {
                ...
                if ($fromObject -ne $toObject) {
                    ...
                    # remap content reference
                    $contentItem.ProducedBy = $toObject

                    # add the content to the toObject
                    $toObject.Produced.Add($contentItem)
        ...
        if (($null -ne $toObject) -and ($fromObject -ne $toObject)) {
            ...
            $this.Studios.Remove($fromObject)
        }
